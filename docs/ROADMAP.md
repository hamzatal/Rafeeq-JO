...
# ملحق A — تطوير نموذج النقل: Zone-based Pooling + مزايا متقدمة + مكافحة الاحتيال
> أُضيف بناءً على متطلبات المالك. الحالة: **مُعتمد كتصميم، قيد التنفيذ التدريجي.** يُدمج ضمن Phase 2/3/5.

## A.1 النموذج: تجميع حسب المنطقة (باب لباب)
- **Zones**: تقسيم جغرافي للمدينة (يبدأ بإربد). كل zone = مضلّع/مركز + نصف قطر.
- **RideRequest** (طلب ركوب): student_id, pickup_lat/lng (موقع البيت), university_id (الوجهة), desired_time, type[scheduled|express], zone_id, status[pending|grouped|assigned|completed|cancelled].
- **Pooling/Matching Engine**: يجمّع طلبات نفس (zone + university + نافذة زمنية) في Trip واحد لكابتن، ويرتّب الالتقاط بأفضل تسلسل (تحسين مسار/أقرب-فأقرب، لاحقاً AI Route Intelligence).
- توسعة `trip_passengers`: `pickup_lat`, `pickup_lng`, `dropoff_confirmed_at`.
- جدول `zones`: name_ar/en, polygon/center+radius, city, is_active.

## A.2 الطلب المستعجل (Express)
- نوع طلب `express`: **خاص** (كابتن فوري، سعر أعلى) أو **أولوية** (ينضم لأقرب مجموعة + رسوم استعجال).
- AI يتنبّأ بوقت الانتظار ويعرض بدائل (استنى/استعجل).
- حقول: `priority_level`, `express_fee_fils`.

## A.3 التتبّع المباشر (Realtime عبر Laravel Reverb)
- قنوات: `trip.{id}` (موقع/حالة)، `user.{id}` (إشعارات)، `admin.trips` (خريطة حيّة).
- الكابتن يبث الموقع → الطالب + الإدارة يستقبلوا لحظياً. (يستبدل الـ polling الحالي.)

## A.4 اقتصاد المقاعد الفاضية (منع خسارة الكابتن/المالك)
- **Min-fill threshold**: لا تُدفع الرحلة للكابتن قبل حد أدنى من الركّاب، أو تتحوّل لتسعير استعجال.
- **تسعير ديناميكي** ضمن سقف معلن + ضمان حد أدنى من صندوق العمولة.
- **معاينة الأرباح المتوقعة** للكابتن قبل القبول.
- النموذج المسبق الدفع يقلّل الأثر.

## A.5 نظام مكافحة الاحتيال (Anti-Disintermediation) — أولوية قصوى
1. **الدفع المسبق للمنصة** (اشتراك/محفظة) — الكابتن يتقاضى من المنصة لا كاش. (أقوى رادع للتسريب.)
2. **OTP صعود + OTP إنزال** (تأكيد الطرفين داخل التطبيق لإكمال الرحلة).
3. **كشف الرحلة الشبح (GPS):** رحلة "ملغاة" + موقع الكابتن يتبع المسار قرب نفس الطلاب → علم.
4. **AI Fraud Monitor:** كشف أنماط (إلغاء متكرر، تكرار نفس الكابتن+الطلاب، تقييمات متبادلة، تطابق مواقع بعد الإلغاء).
5. **Risk Score** لكل حساب → عتبات → تجميد + تحقيق (مرتبط بنظام الشكاوى 9 طبقات).
6. جداول: `risk_flags`, `fraud_events`, `cancellation_logs` (سبب/وقت/موقع).
7. تسجيل GPS وOTP كأدلة في **مركز النزاعات**.

## A.6 المحفظة مسبقة الدفع (Rafeeq Wallet) — تقديمها مبكراً
- `wallets`, `wallet_transactions` (شحن/خصم/استرداد/عمولة). الشحن عبر CliQ (Phase 3).
- الرحلة تُخصم من الرصيد/الاشتراك؛ عمولة المنصة محجوزة تلقائياً.

## A.7 مزايا داعمة معتمدة
- خيار **رحلة/كابتن نسائي** (gender preference) — أمان وثقة.
- **غرامة No-show** للطرفين + سياسة إلغاء عادلة + نوافذ سماح.
- **تقييم ثنائي الاتجاه** (كابتن↔طالب).
- **حوافز الكباتن** بأوقات الذروة/الامتحانات.
- **مشاركة الرحلة** مع ولي الأمر + SOS.
- **fallback SMS** للإشعارات الحرجة.
- **الأداء:** Redis cache + فهارس + WebSockets + queues + pagination.

## A.8 ترتيب التنفيذ المقترح
1. Zones + RideRequests + توسعة trip_passengers (pickup_lat/lng, dropoff OTP).
2. Pooling/Matching Engine (نسخة أولى: زون+جامعة+نافذة زمنية، تسلسل أقرب-فأقرب).
3. Wallet مسبق الدفع + احتساب العمولة (تأسيس، تكامل CliQ في Phase 3).
4. Reverb realtime (تتبّع حيّ + خريطة إدارة).
5. Express rides + تسعير ديناميكي + min-fill.
6. AI Fraud Monitor + Risk Score + cancellation/GPS analytics + مركز النزاعات.
7. مزايا: نسائي، No-show، تقييم ثنائي، حوافز، مشاركة الرحلة، SMS fallback.


---

## خارطة الطريق التجارية والهندسية (Business Roadmap)

# خارطة الطريق (Roadmap)

## مراحل التوسع التجاري
| المرحلة | الخدمة |
|---------|--------|
| 1 | النقل الجامعي |
| 2 | إرسال الأغراض |
| 3 | التبادل الطلابي |
| 4 | النقل بين المحافظات |
| 5 | النقل المؤسسي للشركات |
| 6 | عقود الجامعات |
| 7 | محفظة رقمية |
| 8 | منصة خدمات جامعية متكاملة |

## مراحل التنفيذ التقني (Engineering Phases)
| Phase | النطاق | الحالة |
|-------|--------|--------|
| 0 | الأساس: Monorepo، Backend foundation، Design system | ✅ |
| 1 | الهوية والأمان: Auth / OTP / RBAC / Documents | 🔄 |
| 2 | النقل: Universities / Areas / PickupPoints / Routes / Subscriptions / Trips / Tracking | ⏳ |
| 3 | الدفع: Payment Requests / CliQ / GPT Vision / Financial Transparency | ⏳ |
| 4 | الخدمات: Parcels / Lost & Found / Rewards | ⏳ |
| 5 | الذكاء الاصطناعي: Rafeeq Assistant / Support / Complaint / Safety | ⏳ |
| 6 | لوحة الإدارة + Analytics + Audit Logs | ⏳ |
| 7 | الإطلاق: SOS / Notifications / Deployment / Hardening | ⏳ |

## نقاط القوة التنافسية
- أول منصة تجمع النقل والخدمات الطلابية.
- نظام أمان متعدد الطبقات.
- دعم ذكي مع GPT.
- إرسال أغراض عبر شبكة الرحلات.
- نظام مفقودات ذكي.
- تحليلات ذكاء اصطناعي.
- قابلية توسع عالية + إمكانية التعاقد مع الجامعات.
- نموذج تشغيلي قابل للاستثمار.
- بنية تسمح بإضافة خدمات جديدة دون إعادة بناء النظام.
