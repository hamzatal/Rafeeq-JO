...
**Indexes:** `action` · `auditable_type,auditable_id` · `user_id`
...
**Indexes:** `actor_user_id,created_at` · `trip_id`
...
**Indexes:** `coupon_id,user_id` · `coupon_id,user_id`
...
**Indexes:** `driver_id,resolved` · `expires_at` · `resolved` · `trip_id`
...
**Indexes:** `bank_reference` (unique) · `bank_reference` · `image_hash` · `payment_request_id` · `status`
...
**Indexes:** `is_active` · `university_id`
...
**Indexes:** `ratee_id` · `rater_id` · `trip_id,rater_id,direction` (unique)
...
**Indexes:** `direction` · `status` · `student_id,status` · `subscription_id` · `trip_id` · `zone_id,university_id,status`
...
| `key` | varchar | — |  |
| `value` | text | ✓ |  |
| `group` | varchar | — | 'general'::character varying |
| `updated_by` | uuid | ✓ |  |
| `created_at` | timestamp | ✓ |  |
| `updated_at` | timestamp | ✓ |  |
...
**Indexes:** `status` · `trip_id` · `user_id`
...
**Indexes:** `is_active` · `route_id` · `university_id`
...
**Indexes:** `driver_id` · `plate_number` (unique)
...
**Indexes:** `reversal_of` · `wallet_id,created_at` · `wallet_id,type,reference`
...
**Indexes:** `is_active` · `zone_id,university_id` (unique)
