...
**✅ تم (RFQ-231 → RFQ-234) — إكمال قائمة المزايا (1-5):**
...
- **ترجمة backend واعية بالـ locale (RFQ-234, RFQ-236):** **كل الـ 25 enum** صارت ثنائية اللغة (labelEn + trait)، وكل `*_label` بكل API resources تتبع `Accept-Language`. يبقى فقط ترجمة نصوص أجسام صفحات اللوحة الثابتة (عناوين/أزرار) — العناصر الديناميكية والهيكل مترجمة.
