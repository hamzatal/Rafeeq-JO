...
| | |
|---|---|
| الفرع الحالي | `foundation/phase-0-1` (+ نسخة `main`) |
| آخر Commit | RFQ-292 |
| نسبة الإنجاز | ~90% (كود) · **أمان حرج مُعالَج + فهارس قاعدة بيانات + تصميم Onyx موحّد + توثيق مرتّب** · **166 اختبار (كلها ناجحة)** |
| المرحلة الحالية | **بعد إصلاحات الأمان/الأداء وتوحيد التصميم على Onyx. التصميم مُجمَّد حالياً (تحسينه لاحقاً). المتبقّي: نطاق مزايا MVP + جودة الكود (PHPStan/Pint) + التكاملات الخارجية + الإطلاق** |

---

## الخطوة التالية (ابدأ من هنا) ▶️
> **العمل الحالي:** أُنجز بحث السوق (50+ تطبيق) + مراجعة المزايا + إصلاح أيقونات RTL + تنسيق Pint. **التصميم مُجمَّد على Onyx بقرار المستخدم** (تحسينه آخر خطوة، بالاستناد لخارطة تصميم مستوحاة من البحث). **التالي:** استكمال PHPStan (توثيق `@property` للنماذج) + تطبيق مزايا الأمان المقترحة (شارك رحلتي / وضع الطالبات) عند الجاهزية.

**✅ RFQ-292 — جودة الكود (Pint + تحليل PHPStan):**
- **Pint:** توحيد نمط الكود عبر **109 ملف** (`no_unused_imports`, `ordered_imports`, `braces_position`...) — **166 اختبار أخضر بعده**.
- **PHPStan (larastan level 5):** 242 ملاحظة مُصنَّفة — 180 منها خصائص Eloquent سحرية (تحتاج توثيق `@property` للنماذج) و~62 مسألة نوع حقيقية. **ليست بوابة CI**؛ خُطّط استكمالها كمهمّة توثيق نماذج مخصّصة (بلا تعديل جماعي متسرّع محفوف بالمخاطر).

**✅ RFQ-291 — بحث سوق (50+ تطبيق) + مراجعة مزايا + إصلاح أيقونات + تقرير تصميم:**
- **`docs/MARKET_AND_FEATURE_STUDY.md`:** دراسة تفصيلية موثّقة لـ Uber/Careem/inDrive/Bolt/Grab/DiDi/Ola/Yango/Jeeny... — نماذج التسعير، أنماط التصميم (خريطة+شيت، زر مهيمن، server-driven)، مزايا الأمان، ومصادر GitHub مفتوحة.
- **مراجعة مزايا رفيق:** ما نطوّره (تتبّع/SOS/تسعير/ملف) · ما نضيفه (وضع الطالبات، رحلات مجدولة، «اقترح سعرك»، ETA سياقي، تقسيم الأجرة) · ما نبسّطه (تبادل/مفقودات/مساعد — إخفاء بـ flag بلا حذف).
- **إصلاح أيقونات ضاربة:** جعل `Icon.tsx` (للتطبيقين) **واعياً بالـ RTL** فينعكس الاتجاهي (chevron/arrow/log-out/send) بالعربي — كان يشير للاتجاه الخاطئ. **typecheck نظيف.**
- خارطة تصميم مستوحاة من البحث جاهزة للتطبيق عند رفع تجميد التصميم.

**✅ RFQ-290 — ترتيب التوثيق + مزامنة نظام RFQ + فرع `main`:**
- تحديث فهرس `docs/README.md` ليعكس الواقع (Onyx، 65 جدول، 166 اختبار) وإزالة المعلومات القديمة (v6/50 جدول/156 اختبار).
- **مزامنة العمل الأخير مع نظام ترقيم RFQ** في هذا السجلّ (RFQ-286 → RFQ-290) بنفس التنسيق.
- **تجميع كل الشغل على الفرع الرئيسي + إنشاء فرع `main`** ليسحب المستخدم كل شي بـ `git pull` واحد (كان الفرع الافتراضي `foundation/phase-0-1` والتقارير على فرع منفصل).

**✅ RFQ-289 — تدقيق شامل + دراسة مزايا + معرض 30 تصميم:**
- `docs/FULL_PROJECT_AUDIT_2026-07.md`: حالة المشروع الكاملة + المشاكل + خطة العمل.
- `backend/SECURITY_QA_AUDIT.md`: تدقيق أمني/جودة تفصيلي بمراجع الأسطر.
- `docs/FEATURE_STUDY.md`: تقييم المزايا الـ32 + توصيات نطاق MVP للسوق الأردني (خيار أمان الطالبات كتمييز).
- `docs/DESIGNS.md` + `docs/designs/*.svg`: **30 اتجاه تصميم إبداعي** (أشكال/لاي-آوت/رموز مختلفة) كصور SVG تُعرض داخل GitHub — للاختيار المستقبلي.

**✅ RFQ-288 — تحسين قاعدة البيانات: فهارس المسارات الساخنة:**
- تحليل الـ75 هجرة برمجياً ورصد 55 عمود FK مرشّح بلا فهرس (Postgres لا يفهرس FK تلقائياً).
- إضافة فهارس للأعمدة المستخدمة فعلاً فقط (رحلات الكابتن، المطابقة، اشتراكات، كوبون لكل مستخدم، فضّ الصلاحيات، SOS...) مع **تجاهل متعمّد** لأعمدة الإجراءات الإدارية النادرة (توفيراً لكلفة الكتابة).
- قرار: لا حذف جداول (مزايا مؤجّلة فيها كود)؛ أنواع المال الحالية كافية. **166 اختبار أخضر + migrate نظيف.**

**✅ RFQ-287 — معالجة ثغرات الأمان الحرجة (قبل التعامل بأموال حقيقية):**
- **بثّ التتبّع الحيّ صار خاصاً** (`PrivateChannel` + `routes/channels.php` بمصادقة sanctum + فئات تفويض مختبَرة): الكابتن/الركّاب/الطاقم فقط.
- **حجوزات المحفظة اليتيمة:** `end()` يحرّر حجز الراكب الذي لم يصعد ويعلّمه no-show + تغليف `end()/cancel()` بمعاملات.
- **سدّ تجاوز MFA** عبر مسار OTP بلا كلمة مرور (challenge بلا token للحسابات المفعّلة).
- **الكوبون لا يخلق رصيداً وهمياً:** حدّ خصم الرحلة بالعمولة (دفتر متوازن) + إعادة فحص per-user/first-order تحت القفل.
- SOS: `throttle:sensitive` + تحقّق ملكية الرحلة؛ حارس OTP إنتاجي صلب؛ throttle لإنشاء الدفع.
- **+7 اختبارات أمان جديدة** (تفويض القنوات، تحرير الحجز، MFA-OTP، zero-sum للكوبون).

**✅ RFQ-286 — توحيد الهوية على نظام Onyx + تنظيف الحجم:**
- **إنهاء فوضى التصميم** (كانت 4 هويّات متعايشة: v7 Onyx + v6 كحلي/ذهبي + بقايا lime/v3) بتوحيد كل الأسطح على **Onyx** (إنك `#0A0D12` + أزرق `#2F6BFF`) عبر التطبيقات الثلاث.
- سبلاش/شاشة خطأ/AuthShell/ترحيب/خرائط/إشعارات + رحلة مصادقة الطالب (register/otp) موحّدة؛ حذف `AuthHeader`؛ الشعار «رفيق» عربي موحّد؛ إعادة تلوين لوحة الأدمن.
- تنظيف: إزالة تتبّع zip بحجم 15.5MB + صور ميتة (~16MB) + تعليقات قديمة + تصحيح Laravel 11→12.
