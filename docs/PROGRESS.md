...
- **ترجمة backend واعية بالـ locale (RFQ-234):** `labelEn()` + trait للـ enums الأساسية، والـ labels تتبع Accept-Language (تبديل لغة اللوحة/التطبيق يبدّل نصوص الحالات/الأنواع).
- متبقٍّ من الترجمة: نصوص أجسام صفحات اللوحة الثابتة + بقية الـ enums.
