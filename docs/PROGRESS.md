...
| | |
|---|---|
| الفرع الحالي | `feature/admin-dashboard-fixes-and-cliq` (من `foundation/phase-0-1`) |
| آخر Commit | RFQ-221 |
| نسبة الإنجاز | ~99% (كود) · **فحص احترافي شامل + إصلاحات لوحة الأدمن وتدفّق CliQ (PR #13)** · التشغيل الفعلي مُتحقَّق على PostgreSQL 16 (122 اختبار + E2E حيّ) — راجع `docs/LAUNCH_CHECKLIST.md` للمتبقّي التشغيلي |
...
**✅ تم (RFQ-217 → RFQ-221) — فحص احترافي شامل + إصلاحات لوحة الأدمن وتدفّق CliQ (PR #13):**

> **سياق:** فحص شامل (شغّل 114 اختبار، رُكّب Larastan لتحليل ساكن على كل الكود، قُرئت 59 migration وخدمات الـ AI والتكامل) + إصلاح المشاكل المُبلّغة من التجربة الحيّة على الهاتف.

- **🔴 إصلاح باگ تشغيلي حرج (RFQ-217):** موديل `Trip` كان ينقصه علاقات `university()/vehicle()/zone()` → `DriverTripController::offers()` و`acceptOffer()` يرميان `RelationNotFoundException` (تدفّق عروض الكابتن المجمّع door-to-door كان ينهار). أُضيفت العلاقات + `@property` دقيقة + اختبار يغطّي التدفّق.
- **لوحة الأدمن — اتجاه وi18n (RFQ-217):** السايدبار والمحتوى يقلبان يسار/يمين حسب اللغة (كانا مثبّتين يمين)، إعادة تصميم السايدبار + ترجمة الهيكل (قاموس i18n محلي).
- **بحث وإشعارات فعّالة (RFQ-217):** الشريط العلوي صار بحثه ينقل لصفحة المستخدمين، والإشعارات عدّاد غير مقروء + قائمة منسدلة + تعليم الكل مقروء.
- **الملف الشخصي + شحن المحفظة (RFQ-217):** صفحة `/profile` (تعديل الاسم/الإيميل + كلمة المرور)، ونافذة شحن محفظة الطالب/الكابتن (`/admin/wallets/credit`).
- **إدارة فريق الأدمن (RFQ-218):** وحدة backend (StaffService/Controller) + مسارات `/admin/staff` محميّة بـ `users.manage` (أدمن فقط) + صفحة `/admins` (إضافة/تعديل موظف وأدوار). 4 اختبارات.
- **قسم CliQ + تغيير الـ alias (RFQ-219):** وحدة `Settings` جديدة (جدول settings + خدمة بـ fallback لـ config) → الـ alias قابل للتغيير وقت التشغيل من `/admin/settings/cliq` (صلاحية `settings.manage`)، وصفحة `/cliq` (إعدادات + طلبات الشحن). 2 اختبار.
- **تحقق الـ AI من اسم المُرسِل (RFQ-220):** `PaymentVerificationService` صار يمرّر اسم صاحب الحساب لـ GPT-Vision ويستخرج `sender_name`+`name_matches`؛ الاعتماد التلقائي يتطلب تطابق المبلغ **والاسم**، وإلا مراجعة بشرية (إشارة احتيال). 2 اختبار.
- **رفع الإيصال على الموبايل (RFQ-221):** كان رفع إيصال CliQ بتطبيق الطالب **ويب-فقط** → أُضيف `expo-image-picker` ودالة رفع عبر-منصّات (كاميرا/معرض على الموبايل).
- **ملاحظة بيئية:** خطأ الكوبونات (`no such table: coupons`) سببه قاعدة محلية ناقصها آخر migrations — الحل: `php artisan migrate`.
- **تنبيه أمني:** `composer audit` يُظهر 3 ثغرات على `laravel/framework` v11 (أخطرها CRLF injection high) → يُنصح بترقية الإطار بفرع منفصل.
- التحقق: **122 اختبار خضراء** + type-check نظيف (admin-dashboard + api-client + student-app).
