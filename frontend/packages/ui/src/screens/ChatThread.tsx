/* ═══════════════════════════════════════════════════════════════════════════
   THE TRIP CHAT — one copy.

   ── Why this moved ─────────────────────────────────────────────────────────

   `student-app/app/(app)/chat.tsx` and `driver-app/app/(app)/chat.tsx` were
   BYTE-IDENTICAL: same 161 lines, same md5. Two copies of a polling loop, a
   `FlatList`, an optimistic send and a failure banner, and nothing compared them —
   which is the mechanism that let the two tab bars break the same approved decision
   in two different ways and neither be right (see the note at the top of this
   package's barrel).

   The `duplicated-app-file` gate in `scripts/check-invariants.mjs` only scans each
   app's `src/` tree, so a duplicate under `app/` was invisible to it. It is a route
   file, and route files were assumed to be app-specific. This one was not.

   ── Why `api` is a prop ────────────────────────────────────────────────────

   It is the ONLY thing the two versions could have differed in: the client is
   constructed per app with that app's token storage key. Everything else — the
   locale, the theme, the copy — already came from context, which is exactly why the
   two files were byte-equal rather than merely similar.
   ═══════════════════════════════════════════════════════════════════════════ */
 ...
 */
export function ChatThread({ api }: { api: RafeeqApi }) {
  const { t } = useI18n();
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top', 'bottom']}>
      <View style={s.header}>
        ...
        <Text role="titleMd" align="center" numberOfLines={1} style={s.flex}>{params.title ?? t('chat.title')}</Text>
      ...
      <KeyboardAvoidingView style={s.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={80}>
        <FlatList
          ...
          ListEmptyComponent={<Text role="body" tone="secondary" align="center" style={s.empty}>{t('chat.empty')}</Text>}
          renderItem={({ item }) => (
            <View style={[s.bubbleRow, item.mine ? s.mineRow : s.theirsRow]}>
              <View style={[s.bubble, item.mine ? s.mine : s.theirs]}>
                <Text role="bodyLg" tone={item.mine ? 'inverse' : 'default'}>{item.body}</Text>
                {item.created_at ? (
                  <Text role="caption" tone={item.mine ? 'inverse' : 'secondary'} align="left">
