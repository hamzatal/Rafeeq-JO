...
/* ═══════════════════════════════════════════════════════════════════════════
   Two components, one per platform, because HOOKS CANNOT BE CONDITIONAL.

   This was ONE component with `if (Platform.OS === 'web') { … return … }` in the
   middle, and three hooks after that return: `useState(googleFailed)`,
   `useMemo(html)` and the live-update `useEffect`. On web those three never ran; on
   native they did.

   It did not crash, and that is exactly why it survived: `Platform.OS` is constant
   for the life of the process, so the hook order is stable *per platform*. The bug
   is latent, not absent — the first time anything else becomes conditional, or the
   first time this renders under a platform shim that flips, React reads the wrong
   slot and the map's state becomes another hook's state. It also meant
   `require('react-native-webview')` sat in the middle of a render path.

   Found by turning on `react-hooks/rules-of-hooks`, which had never run here: this
   workspace had no ESLint configuration at all.
   ═══════════════════════════════════════════════════════════════════════════ */

/** Everything both platforms need. One hook, so neither can drift from the other. */
function useMapCommon({ points, route, legend, onPick }: LiveMapProps) {
  ...
  const colors = useMemo(
    () => ({
      ...
      /*
       * `brand[500]`, not `#2F6BFF`.
       *
       * That literal was here, in `MapBackdrop`'s default and in the receipt PDF —
       * three places, one off-brand blue, close enough to `#1259E3` that nobody
       * questioned it. The pickup marker has to differ from the captain marker, so
       * it is a lighter step of the SAME ramp rather than a different hue.
       */
      pickup: brand[500],
  ...
  const kinds = useMemo(() => new Set(points.map((pt) => pt.kind ?? 'pickup')), [points]);
  const showLegend = legend ?? kinds.size > 1;

  /* Built once, so the iframe / WebView is not torn down on every render. */
  const initial = useRef({ points, route, center }).current;

  return { theme, t, s, colors, kinds, showLegend, initial, hasPick: Boolean(onPick) };
}

export function LiveMap(props: LiveMapProps) {
  return Platform.OS === 'web' ? <WebLiveMap {...props} /> : <NativeLiveMap {...props} />;
}

/** Web: a real Leaflet + OpenStreetMap map inside an `<iframe>`. */
function WebLiveMap({ points, route, onPick, legend, height = 220 }: LiveMapProps) {
  const { theme, t, s, colors, kinds, showLegend, initial, hasPick } = useMapCommon({
    points,
    route,
    onPick,
    legend,
  ...
  const html = useMemo(
    () => buildLeafletHtml(initial.points, initial.route, initial.center, colors, hasPick),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [colors, hasPick],
  );

  /* Push live markers into the running map. */
  useEffect(() => {
    frameRef.current?.contentWindow?.postMessage(
      JSON.stringify({ __rafeeqUpdate: true, payload: { points, route: route ?? null } }),
      '*',
    );
  }, [points, route]);

  /* Receive tap-to-pick coordinates back out of the iframe. */
  useEffect(() => {
    if (!onPick) return;
    const handler = (e: MessageEvent) => {
      try {
        ...
        /* a message from something else on the page */
      }
    };
  ...
  return (
    <View>
      <View style={[s.mapWrap, { height }]}>
        ...
        {onPick ? <PickHint s={s} theme={theme} label={t('map.pickHint')} /> : null}
      </View>
...
/**
 * Native: a WebView. Official Google Maps when a key is configured, else Leaflet.
 *
 * `require` at module scope would pull `react-native-webview` into the web bundle,
 * where it does not resolve — so it stays a lazy require, but now at the top of a
 * component that only ever renders on native rather than after an early return.
 */
function NativeLiveMap({ points, route, onPick, legend, height = 220 }: LiveMapProps) {
  const { theme, t, s, colors, kinds, showLegend, initial, hasPick } = useMapCommon({
    points,
    route,
    onPick,
  ...
  /* Google failing to authenticate must not leave a blank rectangle. */
  const [googleFailed, setGoogleFailed] = useState(false);
  const useGoogle = Boolean(mapsKey) && !googleFailed;

  const html = useMemo(
    () =>
      useGoogle
        ? buildGoogleHtml(initial.points, initial.route, initial.center, colors, hasPick, mapsKey)
        : buildLeafletHtml(initial.points, initial.route, initial.center, colors, hasPick),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [useGoogle],
  );
...
/** The «اضغط على الخريطة» overlay. Was duplicated in both branches. */
function PickHint({ s, theme, label }: { s: ReturnType<typeof makeStyles>; theme: AppTheme; label: string }) {
  return (
    ...
    <View style={s.legend}>
      {[...kinds].map((k) => (
        <View key={k} style={s.legendItem}>
          ...
          <Text role="caption" tone="secondary">{labels[k] ?? k}</Text>
        </View>
      ))}
    </View>
  );
