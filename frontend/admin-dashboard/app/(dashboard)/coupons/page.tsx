...
export default function CouponsPage() {
  ...
  const remove = (c: Coupon) => {
    if (confirm(`حذف الكوبون ${c.code}؟`)) api.admin.deleteCoupon(c.id).then(load);
  };
  ...
  return (
    <div>
      <h1 className="text-2xl font-extrabold surface-text mb-4">الكوبونات والخصومات</h1>

      <div className="card mb-5">
        <h2 className="font-bold surface-text mb-3">إنشاء كوبون</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          <input className="input" placeholder="الرمز (WELCOME)" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} />
          ...
          <input className="input" type="number" placeholder={form.type === 'percentage' ? 'النسبة %' : 'القيمة د.أ'} value={form.value} onChange={(e) => setForm({ ...form, value: e.target.value })} />
          <select className="input" value={form.scope} onChange={(e) => setForm({ ...form, scope: e.target.value as CouponScope })}>
            {SCOPES.map((s) => (
              <option key={s.value} value={s.value}>{s.label}</option>
            ))}
          </select>
          <input className="input" type="number" placeholder="حد أقصى للخصم (د.أ)" value={form.max_discount_jod} onChange={(e) => setForm({ ...form, max_discount_jod: e.target.value })} />
          <input className="input" type="number" placeholder="حد أدنى للمبلغ (د.أ)" value={form.min_amount_jod} onChange={(e) => setForm({ ...form, min_amount_jod: e.target.value })} />
          <input className="input" type="number" placeholder="حد الاستخدام الكلي" value={form.usage_limit} onChange={(e) => setForm({ ...form, usage_limit: e.target.value })} />
          <input className="input" type="number" placeholder="حد لكل مستخدم" value={form.per_user_limit} onChange={(e) => setForm({ ...form, per_user_limit: e.target.value })} />
          <input className="input" type="date" placeholder="تاريخ الانتهاء" value={form.expires_at} onChange={(e) => setForm({ ...form, expires_at: e.target.value })} />
          <input className="input sm:col-span-2" placeholder="الوصف (اختياري)" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
        ...
        <button disabled={busy} onClick={add} className="btn-primary mt-3">إنشاء</button>
      </div>
      ...
      <div className="card p-0 overflow-hidden">
        {loading ? (
          ...
          <div className="p-6 text-center text-muted">لا توجد كوبونات</div>
        ) : (
          <table className="w-full text-sm">
            <thead className="table-head">
              <tr>
                ...
                <th className="text-right p-3 font-medium">ينتهي</th>
              ...
              {items.map((c) => (
                <tr key={c.id} className="row-line">
                  ...
                  <td className="p-3 text-left">
                    <button onClick={() => remove(c)} className="text-danger text-sm hover:underline">حذف</button>
                  </td>
                </tr>
              ))}
            </tbody>
