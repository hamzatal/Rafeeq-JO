...
export async function getLocationState(): Promise<PermState> {
  try {
    if (Platform.OS === 'web') {
      // Best-effort read of the Permissions API; not all browsers support it.
      ...
      return navigator?.geolocation ? 'unavailable' : 'unavailable';
    }
...
export async function requestLocation(): Promise<boolean> {
  try {
    if (Platform.OS === 'web') {
      // The browser prompts on the first getCurrentPosition call.
 ...
 * Unlike before, this PROACTIVELY requests permission if not yet granted, and
 * supports web — so the map shows the user's real position immediately.
 */
