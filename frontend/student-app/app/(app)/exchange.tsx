...
export default function Exchange() {
  ...
  const typeLabel = (ty: ExchangeType) => t(`exchange.${ty}`);
    ...
    <Screen scroll>
      <View style={s.headerRow}>
        ...
        <Pressable onPress={() => setShowForm((v) => !v)}>
          <Text style={s.link}>＋ {t('exchange.newItem')}</Text>
        </Pressable>
      </View>
          ...
          <View style={s.chips}>
            {TYPES.map((ty) => (
              <Pressable key={ty} onPress={() => setType(ty)} style={[s.chip, type === ty && s.chipActive]}>
                <Text style={[s.chipText, type === ty && s.chipTextActive]}>{typeLabel(ty)}</Text>
              </Pressable>
            ))}
          </View>
      ...
      {items.length === 0 ? (
        <Text style={s.meta}>{t('exchange.none')}</Text>
      ) : (
        items.map((it) => (
          <View key={it.id} style={s.card}>
            <View style={s.row}>
              <Text style={s.cardTitle}>{it.title}</Text>
              <Text style={s.badge}>{typeLabel(it.type)}</Text>
            </View>
            <Text style={s.meta}>
              {it.price_fils ? `${(it.price_fils / 1000).toFixed(3)} ${t('subscriptions.currency')}` : t('exchange.free')}
            </Text>
